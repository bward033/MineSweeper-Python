...
def tile_board(game_board_makers, grid_list):
    list_of_tile=[]
    for i in range(len(game_board_makers)):
        list_of_tile.append([])
        for j in range(len(game_board_makers[i])):
            square = grid_list[i][j].getP1()
            Ypoint = square.getY() + HEIGHT_OF_IMAGES/2
            Xpoint = square.getX() + WIDTH_OF_IMAGES/2

            tile_image = Image(Point(Xpoint,Ypoint),TILE_IMAGE)

            tile_image.draw(window)
            list_of_tile[i].append(tile_image)
...
def draw_board_numbers(game_board_markers, win, grid_list):
...
def convert_to_row_col(click, game_board_markers):
    x_c_point = click.getX()
    y_c_point =click.getY()
    row = (y_c_point - Y_OFFSET) // WIDTH_OF_IMAGES
    column = ( x_c_point - X_OFFSET) // HEIGHT_OF_IMAGES
    #print(row, column)

    return row, column

def expose_empty_cells(game_board_markers, tiles, row, column):
    # uncover tile at row and col

    if 0 <= row <= rows-1 and 0 <= col <= columns - 1:
            tiles[row][col].undraw()
    # determine if the marker at row and col is a number
    # mark row and col as a exposed cell
    # if it was an empty cell call expose_empty_cells on all neighbors
    pass
...
def main():
    ...
    grid_list = draw_the_grid(rows,columns,window) #the mine tile list

    draw_board_numbers(game_board_markers,window,grid_list)

    tiles = tile_board(game_board_markers,grid_list)

    while True:
        click = window.getMouse()

        row, col = convert_to_row_col(click, game_board_markers)
        if 0 <= row <= rows-1 and 0 <= col <= columns - 1:
            tiles[row][col].undraw()
